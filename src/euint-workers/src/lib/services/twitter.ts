...
const Twitter = class Twitter {
    ...
    constructor() {
        ...
        this.client.get("statuses/user_timeline", params, (error:any)=>{
    ...
    public async list(params:{list_id:string,tweet_mode?:string}){

        return await this.client.get('lists/statuses', params);
    ...
    private async makePost (endpoint:string, params:twitter.RequestParams) {
        return new Promise((resolve, reject) => {
            this.client.post(endpoint, params, (error:any, data:any) => {
                if (error) {
                    reject(error);
                } else {
                    resolve(data);
